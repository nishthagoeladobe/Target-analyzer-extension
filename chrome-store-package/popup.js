...
This report was automatically generated by the Adobe Target Inspector Chrome Extension.`
  ...
  generateAuditReportHTML(data) {
    ...
    const generateCircularChart = (value, label, color, isPercentage = false) => {
      ...
      return `
        <div class="audit-chart">
          <svg viewBox="0 0 100 100" class="circular-chart">
            ...
            <text x="50" y="50" class="chart-value">${isPercentage ? value + '%' : value}</text>
          </svg>
...
<body>
  <div class="report-container">
    ...
    <div class="metrics-grid">
      ${generateCircularChart(activitiesCount, 'Activities Detected', '#667eea')}
      ${generateCircularChart(performanceData.pageLoadTime, 'Average Load Time', '#f59e0b')}
      ${generateCircularChart(networkEventsCount, 'Network Events', '#10b981')}
      ${generateCircularChart(activities.filter(a => a.statusCode === 200 || a.statusCode === 'detected').length, 'Successful Calls', '#3b82f6')}
    </div>
      ...
      <div class="audit-score-circle">
        <svg viewBox="0 0 200 200" style="width: 200px; height: 200px; transform: rotate(-90deg);">
          ...
          <circle cx="100" cy="100" r="80" fill="none" stroke="#10b981" stroke-width="12" 
            ...
            stroke-dashoffset="${2 * Math.PI * 80 * (1 - auditScore / 100)}"
            stroke-linecap="round" />
        </svg>
